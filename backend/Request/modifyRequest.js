...
const modifyRequest = async (role, requestId, userId, message) => {
  ...
  try {
    ...
    const eventApprovalQueryUpdation = `UPDATE eventapprovals SET status = $1, updated_at= CURRENT_TIMESTAMP, reason = $2 where event_id = $3 AND approver_id = $4`;
    const eventApprovalParamsUpdation = [
      "Modified",
      message,
      requestId,
      userId,
    ...
    await sendNotification(
      ...
      `Dear respected sir/ma'am,
        Your event titled "${title}" has been rejected by the ${role} for reason ${message}.`
    );
